"""
Test script to reproduce the race condition in upsert_trace (SYNC ORACLE version).
...
4. Thread B: INSERT - FAILS with IntegrityError (unique constraint violated)
"""
...
def concurrent_create_trace(
    ...
    """Run a single concurrent task that tries to create a trace using OracleDb."""
...
def cleanup_trace(db: OracleDb, trace_id: str):
...
def run_race_test(db: OracleDb, num_tasks: int = 10):
    """Run a single race condition test using OracleDb.upsert_trace()."""
    # Use a unique trace_id for this test run
    trace_id = f"race-test-{uuid.uuid4().hex[:8]}"

    print(f"\n{'=' * 60}")
    print("RACE CONDITION TEST (SYNC ORACLE)")
    print(f"{'=' * 60}")
...
def main():
    ...
    # Create OracleDb instance (same pattern as cookbook)
    db = OracleDb(
        db_url=db_url,
