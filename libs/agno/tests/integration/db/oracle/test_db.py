...
def test_get_trace_aggregates_span_counts(oracle_engine):
    ...
    db = OracleDb(db_engine=oracle_engine, traces_table="test_traces_agg", spans_table="test_spans_agg")
    ...
    trace = db.get_trace(trace_id="test-agg-trace")
...
def test_concurrent_upsert_trace_retries_on_unique_violation(oracle_engine):
    ...
    db = OracleDb(db_engine=oracle_engine, traces_table="test_traces_race")
    now = datetime.now(timezone.utc)
    ...
    db.upsert_trace(make_trace("test-race-warmup", "warmup-agent"))

    # Writer A: uncommitted insert of the contended trace_id (its FOR UPDATE read
    # locked nothing because the row did not exist yet)
    blocker = oracle_engine.connect()
    tx = blocker.begin()
    ...
    loser = threading.Thread(target=lambda: db.upsert_trace(make_trace("test-race-1", "loser-agent")))
    loser.start()
    time_module.sleep(2)
    tx.commit()
    ...
    with oracle_engine.connect() as conn:
        agent_id = conn.execute(text("SELECT agent_id FROM test_traces_race WHERE trace_id = 'test-race-1'")).scalar()
