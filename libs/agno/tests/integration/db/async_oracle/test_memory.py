"""Integration tests for AsyncOracleDb memory methods"""
...
@pytest.mark.asyncio
async def test_upsert_and_get_user_memory(async_oracle_db_real):
    ...
    # Upsert memory
    result = await async_oracle_db_real.upsert_user_memory(memory)
    assert result is not None
    assert result.memory_id == "test-memory-1"

    # Get memory back
    retrieved = await async_oracle_db_real.get_user_memory("test-memory-1")
...
@pytest.mark.asyncio
async def test_get_user_memories_with_filters(async_oracle_db_real):
    ...
    for i in range(3):
        ...
        await async_oracle_db_real.upsert_user_memory(memory)

    # Get all memories for user-0
    user_memories = await async_oracle_db_real.get_user_memories(user_id="user-0")
    assert len(user_memories) >= 2

    # Filter by topics
    topic_memories = await async_oracle_db_real.get_user_memories(topics=["topic1"])
    assert len(topic_memories) >= 2


@pytest.mark.asyncio
async def test_delete_user_memory(async_oracle_db_real):
    ...
    await async_oracle_db_real.upsert_user_memory(memory)
    await async_oracle_db_real.delete_user_memory("test-delete-memory")

    # Verify it's gone
    retrieved = await async_oracle_db_real.get_user_memory("test-delete-memory")
    assert retrieved is None


@pytest.mark.asyncio
async def test_delete_multiple_user_memories(async_oracle_db_real):
    ...
    # Delete all at once
    await async_oracle_db_real.delete_user_memories(memory_ids)

    # Verify all are gone
    for memory_id in memory_ids:
        retrieved = await async_oracle_db_real.get_user_memory(memory_id)
        assert retrieved is None


@pytest.mark.asyncio
async def test_get_all_memory_topics(async_oracle_db_real):
    ...
    # Get all topics
    topics = await async_oracle_db_real.get_all_memory_topics()
    assert "ai" in topics
    assert "ml" in topics
...
@pytest.mark.asyncio
async def test_get_all_memory_topics_with_user_id_filter(async_oracle_db_real):
    ...
    alice_topics = await async_oracle_db_real.get_all_memory_topics(user_id="alice")
    assert set(alice_topics) == {"work", "python", "travel"}

    bob_topics = await async_oracle_db_real.get_all_memory_topics(user_id="bob")
    assert set(bob_topics) == {"gaming", "rust"}

    all_topics = await async_oracle_db_real.get_all_memory_topics()
    for topic in ["work", "python", "travel", "gaming", "rust"]:
        assert topic in all_topics


@pytest.mark.asyncio
async def test_get_all_memory_topics_unknown_user_returns_empty(async_oracle_db_real):
    ...
    unknown_topics = await async_oracle_db_real.get_all_memory_topics(user_id="unknown_user")
    assert unknown_topics == []


@pytest.mark.asyncio
async def test_get_all_memory_topics_tenant_isolation(async_oracle_db_real):
    ...
    alice_topics = set(await async_oracle_db_real.get_all_memory_topics(user_id="alice_iso"))
    bob_topics = set(await async_oracle_db_real.get_all_memory_topics(user_id="bob_iso"))
...
@pytest.mark.asyncio
async def test_get_user_memory_stats(async_oracle_db_real):
    ...
    # Get stats
    stats, total = await async_oracle_db_real.get_user_memory_stats()
    assert total >= 2  # At least 2 users
    assert len(stats) >= 2


@pytest.mark.asyncio
async def test_upsert_memories(async_oracle_db_real):
    ...
    results = await async_oracle_db_real.upsert_memories(memories)
...
@pytest.mark.asyncio
async def test_upsert_memories_update(async_oracle_db_real):
    ...
    await async_oracle_db_real.upsert_memories(initial_memories)
    ...
    results = await async_oracle_db_real.upsert_memories(updated_memories)
...
@pytest.mark.asyncio
async def test_upsert_user_memory_merge_is_idempotent(async_oracle_db_real):
    ...
    memories_raw, total_count = await async_oracle_db_real.get_user_memories(user_id="test-user", deserialize=False)
