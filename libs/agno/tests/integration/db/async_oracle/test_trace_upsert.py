"""
Test script to reproduce the race condition in upsert_trace (ASYNC ORACLE version).
...
4. Task B: INSERT - FAILS with IntegrityError (unique constraint violated)
"""
...
async def concurrent_create_trace(
    ...
    """Run a single concurrent task that tries to create a trace using AsyncOracleDb."""
    ...
    try:
        ...
        # Check for the specific IntegrityError (Oracle's unique constraint violation, ORA-00001)
        if "ORA-00001" in error_str or "IntegrityError" in error_str:
            print(f"  Task {task_id:2d}: FAILED - IntegrityError (unique constraint violated)!")
...
async def cleanup_trace(db: AsyncOracleDb, trace_id: str):
...
async def run_race_test(db: AsyncOracleDb, num_tasks: int = 10):
    """Run a single race condition test using AsyncOracleDb.upsert_trace()."""
    # Use a unique trace_id for this test run
    trace_id = f"race-test-{uuid.uuid4().hex[:8]}"

    print(f"\n{'=' * 60}")
    print("RACE CONDITION TEST (ASYNC ORACLE)")
    print(f"{'=' * 60}")
...
async def main():
    # Database configuration - Oracle connection (local Docker)
    db_url = "oracle+oracledb_async://ai:ai@localhost:1521/?service_name=FREEPDB1"

    print(f"Database URL: {db_url}")

    # Create AsyncOracleDb instance
    db = AsyncOracleDb(
        db_url=db_url,
    ...
    try:
        ...
        print("\nNote: Check ERROR logs above for IntegrityError (ORA-00001) errors.")
