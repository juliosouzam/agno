"""Integration tests for AsyncOracleDb metrics methods"""
...
@pytest.mark.asyncio
async def test_calculate_and_get_metrics(async_oracle_db_real):
    ...
    result = await async_oracle_db_real.calculate_metrics()
    # Result can be None or list
    assert result is None or isinstance(result, list)


@pytest.mark.asyncio
async def test_get_metrics_with_date_range(async_oracle_db_real):
    """Test getting metrics with date filters"""
    start_date = date(2024, 1, 1)
    end_date = date(2024, 12, 31)

    metrics, latest_update = await async_oracle_db_real.get_metrics(starting_date=start_date, ending_date=end_date)
