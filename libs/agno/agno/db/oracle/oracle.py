...
class OracleDb(BaseDb):
    def __init__(
        ...
        """
        Interface for interacting with an Oracle database.
        ...
        Args:
            ...
            db_url (Optional[str]): The database URL to connect to
                (e.g. oracle+oracledb://user:pass@host:1521/?service_name=FREEPDB1).
            db_engine (Optional[Engine]): The SQLAlchemy database engine to use.
            db_schema (Optional[str]): The database schema to use. In Oracle a schema
                is a user; it must already exist. Defaults to the connected user's schema.
            ...
            create_schema (bool): Whether to validate the schema exists before creating
                tables. Oracle schemas are users and are never auto-created. Defaults to True.
    ...
    def _create_table(self, table_name: str, table_type: str) -> Table:
        ...
        try:
            ...
            if self.create_schema and self.db_schema is not None:
                with self.Session() as sess:
                    validate_schema_exists(session=sess, db_schema=self.db_schema)
    ...
    def upsert_sessions(
        ...
        try:
            ...
            # inside the same transaction (see agno.db.oracle.utils.merge_upsert).
            with self.Session() as sess, sess.begin():
    ...
    def upsert_memories(
        ...
        try:
            ...
            with self.Session() as sess, sess.begin():
                ...
                # transaction (see agno.db.oracle.utils.merge_upsert).
                memory_ids = []
                for memory in memories:
                    if memory.memory_id is None:
                        memory.memory_id = str(uuid4())
    ...
    def upsert_trace(self, trace: "Trace") -> None:
        ...
        MySQL/Postgres), and the generic `merge_upsert` helper only copies plain values
        into the MERGE -- it does not accept per-column SQL expressions. So, mirroring
        the same pattern already used by `upsert_user_memory`, the existing row (if
        any) is locked with SELECT ... FOR UPDATE, the merged values (earliest
    ...
    def create_span(self, span: "Span") -> None:
        ...
        try:
            ...
            with self.Session() as sess, sess.begin():
                sess.execute(table.insert(), span.to_dict())
    ...
    def create_spans(self, spans: List) -> None:
        ...
        try:
            ...
            with self.Session() as sess, sess.begin():
                ...
                sess.execute(table.insert(), [span.to_dict() for span in spans])
    ...
    def get_learning(
        ...
        raise NotImplementedError("Learning methods not yet implemented for OracleDb")
