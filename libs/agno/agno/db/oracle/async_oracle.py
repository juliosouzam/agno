...
class AsyncOracleDb(AsyncBaseDb):
    def __init__(
        ...
        """
        Async interface for interacting with an Oracle database.
        ...
        Args:
            ...
            db_url (Optional[str]): The database URL to connect to. Should use the async oracledb
                driver (e.g. oracle+oracledb_async://user:pass@host:1521/?service_name=FREEPDB1).
            db_engine (Optional[AsyncEngine]): The SQLAlchemy async database engine to use.
            db_schema (Optional[str]): The database schema to use. In Oracle a schema is a user;
                it must already exist. Defaults to the connected user's schema.
            ...
            create_schema (bool): Whether to validate the schema exists before creating tables.
                Oracle schemas are users and are never auto-created. Defaults to True.

        Raises:
            ValueError: If neither db_url nor db_engine is provided.
        """
        # The Oracle dialect does not accept the json_serializer engine kwarg;
        # JSON serialization is handled explicitly by the OracleJSON column type.
        ...
        if id is None:
            ...
            schema_suffix = db_schema if db_schema is not None else "default"
    ...
    async def table_exists(self, table_name: str) -> bool:
        """Check if a table with the given name exists in the Oracle database.
    ...
    async def _create_table(self, table_name: str, table_type: str) -> Table:
        ...
        try:
            ...
            # Oracle has no CREATE DATABASE/SCHEMA IF NOT EXISTS: a schema is a user
            # and must already exist. Validate instead of trying to create it.
            if self.create_schema and self.db_schema is not None:
                async with self.async_session_factory() as sess:
                    await avalidate_schema_exists(session=sess, db_schema=self.db_schema)
            ...
            if not await self.table_exists(table_name):
                ...
                log_debug(f"Table {self.db_schema or 'default schema'}.{table_name} already exists, skipping creation")

            # Create indexes
            for idx in table.indexes:
                try:
                    # Check if index already exists. Oracle's data dictionary stores
                    # unquoted identifiers uppercased, so names are compared with UPPER().
                    async with self.async_session_factory() as sess:
                        exists_query = text(
                            "SELECT 1 FROM all_indexes WHERE owner = "
                            "COALESCE(UPPER(:schema), SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA')) "
                            "AND table_name = UPPER(:table_name) AND index_name = UPPER(:index_name)"
                        )
                        ...
                        if exists:
                            log_debug(
                                f"Index {idx.name} already exists in {self.db_schema or 'default schema'}."
                                f"{table_name}, skipping creation"
                            )
                            continue
                    ...
                    log_debug(f"Created index: {idx.name} for table {self.db_schema or 'default schema'}.{table_name}")
            ...
            log_error(f"Could not create table {self.db_schema or 'default schema'}.{table_name}: {str(e)}")
            raise
    ...
    async def _get_or_create_table(
        ...
        if not table_is_available:
            ...
            created_table = await self._create_table(table_name=table_name, table_type=table_type)
        ...
        try:
            ...
            async with self.async_session_factory() as sess, sess.begin():
                ...
                if session_name is not None:
                    # Oracle has no ILIKE: extract the JSON field with JSON_VALUE and do a
                    # case-insensitive LIKE. COALESCE to "" avoids dropping rows whose
                    # session_data has no session_name key (JSON_VALUE returns NULL there).
                    # The JSON path must be a SQL literal (a bind raises ORA-40454).
                    stmt = stmt.where(
                        func.lower(
                            func.coalesce(func.json_value(table.c.session_data, literal_column("'$.session_name'")), "")
                        ).like(f"%{session_name.lower()}%")
                    )
                ...
                session_dicts = [dict(row._mapping) for row in records]
                if not deserialize:
    ...
    async def rename_session(
        ...
        try:
            ...
            async with self.async_session_factory() as sess, sess.begin():
                # Oracle has no JSON_SET construct wired up in SQLAlchemy: read-modify-write
                # the session_data dict in Python instead, inside the same transaction
                # (simpler and more portable than a raw json_transform() expression).
                select_stmt = select(table).where(table.c.session_id == session_id).with_for_update()
                if session_type is not None:
                ...
                update_stmt = update(table).where(table.c.session_id == session_id).values(session_data=session_data)
                if session_type is not None:
                    update_stmt = update_stmt.where(table.c.session_type == session_type.value)
                if user_id is not None:
                    update_stmt = update_stmt.where(table.c.user_id == user_id)
    ...
    async def upsert_session(
        ...
        """
        try:
            return await self._upsert_session_attempt(session=session, deserialize=deserialize)
    ...
    async def _upsert_session_attempt(
        self, session: Session, deserialize: Optional[bool] = True
    ) -> Optional[Union[Session, Dict[str, Any]]]:
        """
        ...
        try:
            ...
            if isinstance(session, AgentSession):
                async with self.async_session_factory() as sess, sess.begin():
                    ...
                    # On insert, updated_at mirrors created_at (nothing has updated the row
                    # yet); on a match it is refreshed to now. The ownership check above
                    # already tells us which case this is, since the MERGE row is shared by
                    # both the insert and update branches.
                    updated_at = session_dict.get("created_at") if existing_row is None else int(time.time())
    ...
    async def upsert_sessions(
        ...
        try:
            ...
            # Process each session type in bulk. Oracle has no bulk ON DUPLICATE KEY
            # construct usable through SQLAlchemy, so each row is MERGEd individually
            # inside the same transaction (see agno.db.oracle.utils.build_merge_stmt).
            async with self.async_session_factory() as sess, sess.begin():
        ...
        try:
            ...
            async with self.async_session_factory() as sess, sess.begin():
                ...
                if topics is not None:
                    # Oracle has no JSON_CONTAINS: match each topic as a quoted substring
                    # of the serialized JSON array. type_coerce (not CAST: Oracle rejects
                    # CAST involving CLOB here) reads the column as plain text so the JSON
                    # bind processor does not mangle the LIKE pattern.
                    for topic in topics:
                        stmt = stmt.where(type_coerce(table.c.topics, Text).like(f'%"{topic}"%'))
                if search_content is not None:
                    # Oracle has no ILIKE: case-insensitive search is lower() + LIKE over
                    # the JSON `memory` column read as plain text via type_coerce.
                    stmt = stmt.where(func.lower(type_coerce(table.c.memory, Text)).like(f"%{search_content.lower()}%"))
    ...
    async def upsert_user_memory(
        ...
        try:
            ...
            async with self.async_session_factory() as sess, sess.begin():
                ...
                # A single MERGE binds the same values to both branches, so there is no
                # way to insert with updated_at=created_at and update with updated_at=now
                # in one statement (unlike MySQL's separate INSERT/ON DUPLICATE KEY
                # values). Check for an existing row first to pick the right value.
                existing_result = await sess.execute(
                    select(table.c.memory_id).where(table.c.memory_id == memory.memory_id).with_for_update()
                )
                existing_row = existing_result.fetchone()

                updated_at = memory.created_at if existing_row is None else int(time.time())
    ...
    async def upsert_memories(
        ...
        try:
            ...
            async with self.async_session_factory() as sess, sess.begin():
                # Oracle has no bulk ON DUPLICATE KEY construct usable through
                # SQLAlchemy, so each memory is MERGEd individually inside the same
                # transaction (see agno.db.oracle.utils.build_merge_stmt).
                memory_ids = []
                for memory in memories:
                    if memory.memory_id is None:
                        memory.memory_id = str(uuid4())
                    ...
                    stmt, params = build_merge_stmt(
                        ...
                        update_columns=[
                            ...
                            "feedback",
                            "updated_at",
                            # created_at intentionally NOT listed: preserve on match
                        ],
                    )
                    await sess.execute(stmt, params)
    ...
    async def get_metrics(
        ...
        Returns:
            Tuple[List[Dict[str, Any]], Optional[int]]: A tuple containing the metrics and the timestamp of the
                latest update.
    ...
    async def get_knowledge_content(self, id: str) -> Optional[KnowledgeRow]:
        ...
        try:
            async with self.async_session_factory() as sess, sess.begin():
                ...
                if row is None:
                    return None
                return deserialize_knowledge_row(dict(row._mapping))
    ...
    async def get_knowledge_contents(
        ...
        try:
            async with self.async_session_factory() as sess, sess.begin():
                ...
                return [deserialize_knowledge_row(dict(record._mapping)) for record in records], total_count
    ...
    async def upsert_knowledge_content(self, knowledge_row: KnowledgeRow):
        ...
        try:
            ...
            async with self.async_session_factory() as sess, sess.begin():
                # Get the actual table columns to avoid passing unknown column names
                table_columns = set(table.columns.keys())

                # Only include fields that exist in the table and are not None, so a
                # partial KnowledgeRow (e.g. only id + status) doesn't overwrite
                # existing values with NULL on the MERGE's update branch.
                ...
                # Build insert values and the list of columns to update only for
                ...
                for model_field, table_column in field_mapping.items():
                    if table_column in table_columns:
                        ...
                        if value is not None:
                            ...
                            # Don't include id in update_columns since it's the primary key
                            if table_column != "id":
                                update_columns.append(table_column)
    ...
    async def rename_eval_run(
        ...
        try:
            ...
            async with self.async_session_factory() as sess, sess.begin():
                update_stmt = (
                    update(table).where(table.c.run_id == eval_run_id).values(name=name, updated_at=int(time.time()))
                )
                await sess.execute(update_stmt)
    ...
    async def get_cultural_knowledge(
        ...
        Returns:
            Optional[Union[CulturalKnowledge, Dict[str, Any]]]: The cultural knowledge entry, or None if it
                doesn't exist.
    ...
    async def get_all_cultural_knowledge(
        ...
        try:
            ...
            async with self.async_session_factory() as sess, sess.begin():
                ...
                db_rows = [dict(record._mapping) for record in records]
    ...
    async def upsert_cultural_knowledge(
        ...
        try:
            ...
            return await self.get_cultural_knowledge(id=cultural_knowledge.id, deserialize=deserialize)
    ...
    async def upsert_trace(self, trace: "Trace") -> None:
        ...
        Oracle's SQLAlchemy dialect has no ON CONFLICT / ON DUPLICATE KEY construct
        exposing "excluded"/"inserted" pseudo-columns inside a single statement (unlike
        MySQL/Postgres), and the generic `build_merge_stmt` helper only copies plain
        values into the MERGE -- it does not accept per-column SQL expressions. So,
        mirroring the same pattern already used by `upsert_user_memory`, the existing
        row (if any) is locked with SELECT ... FOR UPDATE, the merged values (earliest
        start_time, latest end_time, recomputed duration, highest-priority root name,
        first non-null context fields) are computed in Python, and the single MERGE
        that follows only ever copies plain values. The row lock closes the race
        window between the read and the MERGE for concurrent upserts of the same
        trace_id.
        ...
        try:
            ...
            def coalesce(existing_value: Any, new_value: Any) -> Any:
    ...
    async def create_span(self, span: "Span") -> None:
        ...
        try:
            ...
            async with self.async_session_factory() as sess, sess.begin():
                await sess.execute(table.insert(), span.to_dict())
    ...
    async def create_spans(self, spans: List) -> None:
        ...
        try:
            ...
            async with self.async_session_factory() as sess, sess.begin():
                # A single executemany-style insert instead of one INSERT per span
                await sess.execute(table.insert(), [span.to_dict() for span in spans])
    ...
    async def get_learning(
        ...
        raise NotImplementedError("Learning methods not yet implemented for AsyncOracleDb")
