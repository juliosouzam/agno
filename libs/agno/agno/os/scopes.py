...
def get_a2a_scope_mappings(prefix: str = "/a2a") -> Dict[str, List[str]]:
    ...
    The ``/v1`` JSON-RPC dispatchers multiplex methods (SendMessage, GetTask, ...) on one
    URL, so the route gate carries the coarsest scope (``:run``) and the dispatcher
    re-checks the per-method scope (GetTask -> ``:read``) inside the handler.
    """
    p = prefix.rstrip("/")
    return {
